...
import type { TrendPostSummary } from "@/lib/api/trend";
...
export function TrendPostList({ agentSlug, posts }: TrendPostListProps) {
      ...
      <Card>
        ...
        <CardContent>
          <Button asChild variant="outline">
            <Link href={ROUTES.agentDetail(agentSlug)}>에이전트로 돌아가기</Link>
          </Button>
        </CardContent>
      </Card>
    );
  ...
  return (
    <ul className="space-y-4">
      {posts.map((post) => (
        <li key={post.post_id}>
          <Link href={ROUTES.trendPost(agentSlug, post.post_id)}>
