...
import { getTrendPostPdfUrl, type TrendPostResponse } from "@/api/trend";
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-3">
            <Button asChild variant="ghost" size="sm" className="w-fit px-0">
              <Link to={ROUTES.trendPosts(agentSlug)}>← 리포트 목록</Link>
            </Button>
            <Button asChild variant="ghost" size="sm" className="w-fit px-0">
              <Link to={ROUTES.agentDetail(agentSlug)}>에이전트로</Link>
            </Button>
          </div>
          <div>
