...
export interface ScrapEmbeddingGraph3DViewProps {
  data: ScrapGraphData;
  minSimilarity: number;
  nodeById: Map<string, ScrapGraphNode>;
  width: number;
  height: number;
...
export function ScrapEmbeddingGraph3DView({
  data,
  minSimilarity,
  nodeById,
  width,
  ...
  onNodeClick,
}: ScrapEmbeddingGraph3DViewProps) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const graphData = useMemo(() => {
    const base = toScrapForceGraph(data, { minSimilarity });
    return {
  ...
  const setupLinkForce = useCallback(() => {
    ...
    linkForce.distance((link: { similarity?: number }) => {
      const sim = link.similarity ?? minSimilarity;
      return 28 + (1 - sim) * 42;
    });
  }, [graphRef, minSimilarity]);
  ...
  const getNodeVal = useCallback(() => 14, []);

  const getLinkColor = useCallback(
    (link: object) => {
      const l = link as {
        source: ScrapForceNode | string;
        target: ScrapForceNode | string;
        similarity?: number;
      };
      ...
      const alpha = inFocus
        ? highlightIds
          ? 0.58 + (sim - minSimilarity) * 0.26
          : 0.36 + (sim - minSimilarity) * 0.18
        : 0.14;
