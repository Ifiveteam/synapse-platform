...
| `repositories/archiver_repository.py` | `ai_chat_logs` CRUD, hybrid RAG |
...
| `archiver/` | Archiver LangGraph (`engine.py` + `steps/`) | [docs/archiver](../archiver/README.md) |

#### Archiver 구조 (다른 에이전트와 상이)
...
- Archiver — [docs/archiver/ARCHITECTURE.md](../archiver/ARCHITECTURE.md)
...
| `indexer.py` | Indexer |
| `archiver.py` | Archiver (SSE, JWT 필수) |
...
| LangGraph 실행 상태 | `agents/{name}/state/` 또는 `agents/{name}/types.py` (Archiver) |
| DB 테이블 | `models/` |
...
| [archiver/ARCHITECTURE.md](../archiver/ARCHITECTURE.md) | Archiver LangGraph·SSE·RAG |
