...
async def node_embed(state: IndexerState) -> IndexerState:
    ...
    try:
        ...
        if skipped:
            logger.info(f"[embed] 미분류 {skipped}건 제외 (임베딩 토큰 미사용)")
        if not items:
        ...
        logger.info(f"[embed] 완료 {len(vectors)}건")
        return {**state, "cleaned_data": enriched, "error": None}
