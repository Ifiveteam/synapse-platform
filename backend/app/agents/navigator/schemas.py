...
class ProfilerLayerB(BaseModel):
    ...
    search_active_ratio: float = Field(
        default=0.0, ge=0, le=1, description="주체성 — 직접 검색 비율 (높을수록 좋음)"
    )
    viewing_concentration: float = Field(
        default=0.0,
        ge=0,
        le=1,
        description="채널 편중도 — 소수 채널 집중도 (높을수록 나쁨)",
    )
    taste_diversity_index: float = Field(
        default=50.0,
        ge=0,
        le=100,
        description="취향 다양성 — 4종 취향 분산 (높을수록 좋음)",
    )
    exploration_depth: float = Field(
        default=0.0,
        ge=0,
        le=1,
        description="탐색 깊이 — 새 주제 진입 시 깊이 (높을수록 좋음)",
    )
...
class IdealRadarChart(BaseModel):
    ...
    direction: str = Field(
        default="", description="방향 요약 (예: practical_orientation→OPPOSITE)"
    )
...
class RadarComparison(BaseModel):
    ...
    def calculate_gap(self) -> "RadarComparison":
        ...
        self.gap = {
            key: round(ideal_dict[key] - current_dict[key], 2) for key in current_dict
        }
...
class IdealDesignResponse(BaseModel):
    ...
    opposite_guide: Optional["Guide"] = Field(
        default=None, description="반대방향형 LLM이 생성한 30일 가이드"
    )
    agent_message: str = ""
