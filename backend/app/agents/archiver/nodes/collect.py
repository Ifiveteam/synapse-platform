...
from app.agents.archiver.models import COLLECT_NODE, ArchiverState
...
async def collect_node(
    ...
    existing_body = (state.get("context_body") or state.get("context_dom") or "").strip()
    ...
    if is_usable_context_body(existing_body):
        writer(status_event(MSG_DOM_COLLECTED))
        patch["context_dom"] = existing_body
        patch["context_body"] = existing_body
        log_collect_result(
            route="BASIC",
            rag_chars=0,
    ...
    if is_thin_context_body(context_body):
        writer(status_event(MSG_DOM_THIN_REVIEW))
