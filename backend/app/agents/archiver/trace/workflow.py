...
def log_workflow_end(state: dict[str, Any], *, latency_ms: int) -> None:
    ...
    banner(
        ...
        f"| eval_score={evaluation.score if evaluation else '—'} "
...
def log_node_enter(node: str, *, state: ArchiverState | None = None) -> None:
    ...
    if node == "router":
        ...
        logger.info(
            "  └─ route=%s | eval_score=%s | sufficient=%s",
            route_label,
