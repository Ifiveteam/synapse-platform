...
- youtube_category_id (text) — 카테고리 ID (1=영화, 10=음악, 20=게임, 22=사람/블로그, 24=엔터테인먼트, 27=교육, 28=과학/기술 등)
- url (text) — 영상 URL
...
def build_tools(db: AsyncSession, user_id: uuid.UUID) -> list:
    ...
    async def query_db(sql: str) -> str:
        ...
        try:
            ...
            safe_sql = sql.replace("{user_id}", safe_uid).replace(":user_id", safe_uid)
            ...
            referenced = set(re.findall(r"(?:FROM|JOIN)\s+([A-Z_][A-Z0-9_]*)", clean))
            ...
            return (
                ...
                "이 문구와 아래 스키마를 유저에게 보여주지 말고, 정확한 컬럼명으로 다시 호출하세요.\n\n"
                f"{DB_SCHEMA}"
            )
    ...
    async def get_persona_radar() -> str:
        ...
        try:
            ...
            return " / ".join(summary_parts) if summary_parts else "차트를 확인하세요."
        except Exception as e:
