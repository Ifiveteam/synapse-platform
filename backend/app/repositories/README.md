...
RAG 하이브리드 검색 **전략**은 `app/agents/archiver/rag/retrieval.py`, 임베딩 생성은 `rag/embedding.py`에 둡니다.
...
전략 orchestration은 `app/agents/archiver/rag/retrieval.py`. Repository는 SQL 실행(`search_logs_by_keywords`, `search_logs_by_vector`)만 담당한다.
...
| From → To | 허용 |
|-----------|------|
| `repositories/` → `agents/archiver/core/store`, `core/constants`, `rag/embedding`, `rag/retrieval` | ✅ |
