...
RAG 하이브리드 검색 **전략**은 `app/agents/archiver/past_knowledge/retrieval.py`, 임베딩 생성은 `past_knowledge/embedding.py`에 둡니다.

## ArchiverRepository

`app/agents.archiver/store.ArchiverStore` Protocol을 구현해 LangGraph `rag_node`에 RAG Port로 주입된다.
...
전략 orchestration은 `app/agents/archiver/past_knowledge/retrieval.py`. Repository는 SQL 실행(`search_logs_by_keywords`, `search_logs_by_vector`)만 담당한다.
...
`ARCHIVER_AGENT_TYPE`은 `app/agents/archiver/core/constants.py` SSOT — repository는 import만 한다.

### Import 경계

| From → To | 허용 |
|-----------|------|
| `repositories/` → `agents/archiver/core/store`, `core/constants`, `past_knowledge/embedding`, `past_knowledge/retrieval` | ✅ |
