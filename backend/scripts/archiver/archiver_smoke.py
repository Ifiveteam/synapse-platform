"""Archiver 런타임 스모크 — Mock LLM으로 4경로 + evaluator 루프 검증."""
...
def _mock_classify(*, is_general: bool, target_engines: list[str]) -> Any:
    async def _fn(state: dict[str, Any]) -> dict[str, Any]:
        ...
        writer(
            {
                "event": "status",
                "content": f"[mock] parallel targets={','.join(target_engines)}\n\n",
            }
        )
        return {
            "is_general": False,
...
async def _run_path(
    *,
    ...
    patches: dict[str, Any] = {
        "app.agents.archiver.workflow.classify": _mock_classify(
...
async def main() -> int:
    ...
    # GENERAL fast-path
    st, tk = await _run_path(label="GENERAL", is_general=True, target_engines=[])
    if not tk or tk[0] != "mock-answer":
        errors.append("GENERAL: token missing")
    if not any(MSG_ROUTER_GENERAL in s for s in st):
